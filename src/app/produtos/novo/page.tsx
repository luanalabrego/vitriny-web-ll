...
export default function NovoProduto() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const tasks = rows.map(row => (async () => {
      try {
        // (upload, gera imagem, persiste...) permanece igual
        // ao final:
        setRows(prev =>
          prev.map(r =>
            r.id === row.id
        ...
        const decJson = await fetch('/api/user/decrement-credits', { method: 'POST' }).then(res => res.json());
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-4 text-black">
        ...
        {rows.length > 0 && (
          <>
                ...
                >
                  {row.preview && (
                    <img
                      src={row.preview}
                      alt="preview"
                      className="h-40 w-full object-cover rounded-xl shadow-md cursor-pointer"
                      onClick={() => setModalImage(row.preview!)}
                    />
                  )}
                  <div className="flex justify-between items-center mt-2">
                    <span className="font-medium text-gray-800">{row.productType}</span>
                    <span className="font-medium text-gray-800">{row.ean}</span>
                  </div>
                  {showDetails && (
                    <div className="mt-2 text-gray-600 text-sm space-y-1">
                      <div><strong>Descrição:</strong> {row.descricao || '-'}</div>
                      <div><strong>Marca:</strong> {row.marca || '-'}</div>
                      <div><strong>Cor:</strong> {row.cor || '-'}</div>
                      <div><strong>Tamanho:</strong> {row.tamanho || '-'}</div>
                    </div>
                  )}
                </div>
              ))}
            ...
            {/* Tabela para desktop */}
            <div className="overflow-x-auto hidden md:block rounded-lg border border-gray-200 overflow-hidden">
                    ...
                    <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="border border-gray-200 p-2">
                        <select
                          ...
                          className="rounded-md border px-2 py-1 w-full bg-white focus:outline-none focus:ring-2 focus:ring-purple-400"
                        >
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-4">
              <button
                type="submit"
                disabled={!canSubmit}
                className="px-4 py-2 w-full sm:w-auto bg-blue-600 text-white rounded-lg shadow transition transform hover:scale-105 disabled:opacity-50"
              >
                Enviar Todas
              </button>
              <button
